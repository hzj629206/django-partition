...
class Command(migrate.Command):
    def execute(self, *args, **options):
        db = options['database']
        if db not in migrate.connections:
            prefix = "%s.write" % db
            for _db in migrate.connections.databases.keys():
                if _db.startswith(prefix) and _db in migrate.connections:
    ...
    def handle(self, *args, **options):
        ...
        db = options['database']
        connection = migrate.connections[db]
        ...
        for partition_database in partition_databases:
            ...
            with connection.cursor() as cursor:
                cursor.execute(partition_database_sql)
